...
class DenseLayer(nn.Module):
    ...
    def __init__(self, in_channels, growth_rate):
        ...
        self.conv1 = nn.Conv2d(in_channels=in_channels, out_channels=4 * growth_rate, kernel_size=1, stride=1, padding=0, bias=False)

        self.bn2 = nn.BatchNorm2d(4 * growth_rate)
        self.conv2 = nn.Conv2d(in_channels=4 * growth_rate, out_channels=growth_rate, kernel_size=3, stride=1, padding=1, bias=False)
...
class DenseNet2D(nn.Module):
    ...
    def __init__(self, densenet_variant, growth_rate, compression_factor, in_channels, num_classes):
        ...
        self.bn2 = nn.BatchNorm2d(dense_block_in_channels)
...
def build_densenet_2d(args):
    ...
    match = re.match(r'([A-Za-z_-]*)DenseNet(\d+)', args.model_name, re.IGNORECASE)
    if match:
...
if __name__ == '__main__':
    ...
    x = torch.randn(1, args.in_channels, args.image_height, args.image_width)
    output = model(x)
