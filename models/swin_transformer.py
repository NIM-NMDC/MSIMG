...
def window_reverse(windows, window_size, H, W):
    ...
    :param W: width of the original tensor
    :return: x: (B, H, W, C)
    """
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        """
        :param x: input tensor of shape (B * num_windows, N, C)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        assert 0 <= self.shift_size < self.window_size, "shift size must in 0 - window size."

        self.norm1 = norm_layer(dim)
        self.attn = WindowAttention(
            dim=dim, window_size=to_2tuple(window_size), num_heads=num_heads,
    ...
    def forward(self, x):
        ...
        # W-MSA / SW-MSA
        attn_windows = self.attn(x_windows, mask=self.attn_mask)  # (B * nW, window_size * window_size, C)
    ...
    def extra_repr(self) -> str:
        return (f"dim={self.dim}, input_resolution={self.input_resolution}, num_heads={self.num_heads}, "
                f"window_size={self.window_size}, shift_size={self.shift_size}, ml=_ratio={self.mlp_ratio}")
...
def build_swin_transformer(args):
    """
    Build a Swin Transformer model based on the provided arguments.

    :param args: Arguments containing model configuration.
    :return: Swin Transformer model.
    """
    swin_transformer_parameters = {
    ...
    config = swin_transformer_parameters.get(args.model_name)

    if config is None:
        valid_models = list(swin_transformer_parameters.keys())
        raise ValueError(
